...
const romanToNumber = roman => {
  ...
  if (roman.length < 2) return lookup[roman[0]];
  let array = [...roman];
  let runningTotal = 0;

  for (let i = 0; i < array.length; i++) {
    if (lookup[array[i + 1]] > lookup[array[i]]) {
      runningTotal += lookup[array[i + 1]] - lookup[array[i]];
      i++;
    } else runningTotal += lookup[array[i]];
  }
...
console.log(romanToNumber('X'));
console.log(romanToNumber('M'));
