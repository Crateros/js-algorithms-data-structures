...
// The rules of Goat Latin are as follows: If a word begins
// with a vowel (a, e, i, o, or u), append "ma" to the end of the word.
...
// starting with 1. For example, the first word gets "a" added to the end,
// the second word gets "aa" added to the end and so on. Return the final sentence
...
const toGoatLatin = S => {
  ...
  for (let i = 0; i < stringArray.length; i++) {
    if (vowels.has(stringArray[i][0].toLowerCase())) {
      stringArray[i] = stringArray[i] + 'ma';
    } else {
      stringArray[i] = stringArray[i].slice(1) + stringArray[i][0] + 'ma';
    }
